...
export function LoginCard({ authStatus, authError }: { authStatus: AuthStatus; authError?: string }) {
  ...
  const fieldClass = "h-11 w-full rounded-xl border border-input bg-card pl-11 pr-4 text-sm text-foreground shadow-sm transition placeholder:text-muted-foreground/75 hover:border-muted-foreground/40 focus-visible:border-primary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/10";

  return (
    <main className="relative flex min-h-dvh items-center justify-center overflow-hidden bg-background p-3 sm:p-6 lg:h-dvh lg:min-h-0 xl:p-8">
      ...
      <section className="relative grid w-full max-w-6xl overflow-hidden rounded-[2rem] border border-border/80 bg-card shadow-[0_28px_90px_rgba(25,48,40,0.14)] lg:h-full lg:min-h-0 lg:max-h-[46rem] lg:grid-cols-[1.08fr_0.92fr]">
        <div className="finance-grid relative hidden overflow-hidden bg-surface-dark text-surface-dark-foreground lg:flex lg:flex-col lg:justify-between lg:p-8 xl:p-10">
          ...
          <div className="relative flex items-center gap-3">
            <div className="flex size-10 items-center justify-center rounded-xl bg-white text-surface-dark shadow-lg shadow-black/20">
          ...
          <div className="relative my-6 max-w-xl">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-semibold text-emerald-300 backdrop-blur">
              <Sparkles className="size-3.5" aria-hidden="true" />
              Tus finanzas, finalmente claras
            </div>
            <h1 className="max-w-lg text-4xl font-semibold leading-[1.08] tracking-[-0.04em]">
              Convierte cada movimiento en una mejor decisión.
            </h1>
            <p className="mt-3 max-w-md text-sm leading-5 text-white/58">
              Entiende lo que ingresa, controla lo que gastas y sigue el progreso de tus metas desde un solo lugar.
            </p>

            <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.07] p-4 shadow-2xl shadow-black/20 backdrop-blur-md">
              ...
              <div className="mt-4 grid grid-cols-12 items-end gap-2" aria-hidden="true">
                {[42, 56, 48, 68, 59, 78, 65, 84, 70, 88, 75, 94].map((height, index) => (
                  <div key={index} className="flex h-16 items-end rounded-full bg-white/5">
                    <span
        ...
        <div className="flex min-h-0 flex-col justify-center px-6 py-8 sm:px-10 lg:py-8 xl:px-12">
          ...
          <form onSubmit={handleCredentials} className="space-y-4">
            ...
            <label className="block space-y-1.5">
              ...
              <span className="relative block">
                <KeyRound className="pointer-events-none absolute left-3.5 top-3.5 size-4 text-muted-foreground" aria-hidden="true" />
                <input
                ...
                <button
                  ...
                  className="absolute right-1.5 top-1.5 flex size-8 items-center justify-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-foreground"
            ...
            <button
              ...
              className="group flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-surface-dark px-4 text-sm font-bold text-surface-dark-foreground shadow-lg shadow-slate-950/10 transition hover:-translate-y-0.5 hover:bg-slate-800 hover:shadow-xl disabled:pointer-events-none disabled:opacity-55"
            >
          ...
          {authStatus.googleConfigured ? (
            <>
              ...
              <button
                ...
                className="flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-input bg-card px-4 text-sm font-bold text-card-foreground shadow-sm transition hover:-translate-y-0.5 hover:bg-muted disabled:pointer-events-none disabled:opacity-55"
              >
          ...
          {!authStatus.ready ? (
            <div role="status" className="mt-4 flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-3.5 text-xs leading-5 text-amber-900">
          ...
          <p className="mt-4 flex items-center justify-center gap-2 text-center text-[11px] leading-5 text-muted-foreground">
