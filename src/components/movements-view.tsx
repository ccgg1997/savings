...
function MovementList({ rows, writable, onEdit, onDelete }: { rows: TransactionRecord[]; writable: boolean; onEdit: (row: TransactionRecord) => void; onDelete: (row: TransactionRecord) => void }) {
  ...
  return (
    <>
      <div className="divide-y divide-border/70 px-4 sm:hidden">{rows.map((row) => <MovementMobileCard key={row.id} row={row} writable={writable} onEdit={onEdit} onDelete={onDelete} />)}</div>
      <div className="hidden overflow-x-auto sm:block">
        <table className="w-full min-w-[980px] text-left text-[11px]">
          <thead><tr className="border-b border-border bg-muted/35 text-[9px] font-bold uppercase tracking-[0.12em] text-muted-foreground"><th className="px-5 py-3">Movimiento</th><th className="py-3">Fecha</th><th className="py-3">División</th><th className="py-3">Categoría</th><th className="py-3">Cuenta</th><th className="py-3 text-right">Monto</th><th className="px-5 py-3 text-right">Acciones</th></tr></thead>
          <tbody>{rows.map((row) => <tr key={row.id} className="group border-b border-border/60 last:border-0 hover:bg-muted/25"><td className="px-5 py-3.5"><MovementIdentity row={row} /></td><td className="whitespace-nowrap py-3.5 text-muted-foreground">{dateFormatter.format(new Date(`${row.date}T12:00:00`))}</td><td className="py-3.5"><span className="rounded-full bg-secondary px-2.5 py-1 font-semibold text-secondary-foreground">{row.division}</span></td><td className="py-3.5 text-muted-foreground">{row.category}</td><td className="py-3.5 text-muted-foreground">{row.account}</td><td className={`whitespace-nowrap py-3.5 text-right font-bold ${row.type === "income" ? "text-emerald-700" : "text-card-foreground"}`}>{row.type === "income" ? "+" : "−"}{money(row.amount)}</td><td className="px-5 py-3.5"><div className="flex justify-end gap-1"><button type="button" disabled={!writable} onClick={() => onEdit(row)} className="flex size-8 items-center justify-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-foreground disabled:pointer-events-none disabled:opacity-30" title="Editar movimiento"><Pencil className="size-3.5" aria-hidden="true" /><span className="sr-only">Editar</span></button><button type="button" disabled={!writable} onClick={() => onDelete(row)} className="flex size-8 items-center justify-center rounded-lg text-muted-foreground transition hover:bg-red-50 hover:text-red-600 disabled:pointer-events-none disabled:opacity-30" title="Eliminar movimiento"><Trash2 className="size-3.5" aria-hidden="true" /><span className="sr-only">Eliminar</span></button></div></td></tr>)}</tbody>
        </table>
      </div>
    </>
  );
...
function MovementMobileCard({ row, writable, onEdit, onDelete }: { row: TransactionRecord; writable: boolean; onEdit: (row: TransactionRecord) => void; onDelete: (row: TransactionRecord) => void }) {
  return <article className="py-4"><div className="flex items-start gap-3"><MovementIdentity row={row} /><div className="ml-auto text-right"><p className={`text-xs font-bold ${row.type === "income" ? "text-emerald-700" : "text-card-foreground"}`}>{row.type === "income" ? "+" : "−"}{money(row.amount)}</p><p className="mt-1 text-[9px] text-muted-foreground">{dateFormatter.format(new Date(`${row.date}T12:00:00`))}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-muted/50 p-3 text-[10px]"><span><span className="block text-[8px] font-bold uppercase text-muted-foreground">División</span><span className="mt-1 block truncate font-semibold text-card-foreground">{row.division}</span></span><span><span className="block text-[8px] font-bold uppercase text-muted-foreground">Categoría</span><span className="mt-1 block truncate font-semibold text-card-foreground">{row.category}</span></span></div>{writable ? <div className="mt-2 flex justify-end gap-1"><button type="button" onClick={() => onEdit(row)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted"><Pencil className="size-3.5" aria-hidden="true" /><span className="sr-only">Editar</span></button><button type="button" onClick={() => onDelete(row)} className="rounded-lg p-2 text-muted-foreground hover:bg-red-50 hover:text-red-600"><Trash2 className="size-3.5" aria-hidden="true" /><span className="sr-only">Eliminar</span></button></div> : null}</article>;
}
