...
export function AdminView({ initialUsers, initialSettings, currentUserId }: { initialUsers: AdminUser[]; initialSettings: AppSettingsData; currentUserId: string }) {
  ...
  async function submitUser(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      notifySuccess(editingUser ? "Usuario actualizado." : "Usuario creado y autorizado.");
    } catch (error) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.55fr_0.85fr]">
        <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
          <div className="flex items-center justify-between border-b border-border px-4 py-3"><div><h2 className="text-sm font-bold text-card-foreground">Usuarios autorizados</h2><p className="mt-0.5 text-xs text-muted-foreground">Google es opcional; una contraseña habilita el acceso local.</p></div><span className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">{users.length}</span></div>
            ...
            <table className="w-full min-w-[780px] text-left text-xs">
              ...
              <tbody>{users.map((user) => (
                <tr key={user.id} className="border-b border-border/70 last:border-0">
                  ...
                  <td><div className="flex flex-wrap gap-1">{user.hasPassword && <span className="rounded-md bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-700">Contraseña</span>}{user.providers.includes("google") && <span className="rounded-md bg-blue-50 px-2 py-1 text-[10px] font-semibold text-blue-700">Google</span>}{!user.hasPassword && user.providers.length === 0 && <span className="rounded-md bg-amber-50 px-2 py-1 text-[10px] font-semibold text-amber-700">Pendiente</span>}</div></td>
      ...
      {editingUser !== undefined && <UserEditor mode={editingUser ? "edit" : "create"} draft={draft} setDraft={setDraft} onClose={closeEditor} onSubmit={submitUser} saving={savingUser} />}
